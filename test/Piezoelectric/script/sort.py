...
output_data = np.empty((len(Electric), len(data_to_sort[0]) * 2))

output_data[:, ::2] = sorted_data
output_data[:, 1::2] = sorted_errors

output_data = np.column_stack((Electric, output_data))
np.savetxt("Evslat-sort.dat", output_data, fmt='%10.6f', delimiter='\t')
